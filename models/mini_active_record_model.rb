module MiniActiveRecord
  ...
  class Model
    ...
    def initialize(attributes = {})
      ...
      @var.attribute_names.each do |name|
    ...
    def self.where(query, *args)
      MiniActiveRecord::Model.execute("SELECT * FROM #{@consulta} WHERE #{query}", *args).map do |row|
        self.new(row)
      end
    end
    ...
    def insert!
      ...
      insert_sql = "INSERT INTO #{@consulta} (#{fields.join(',')}) VALUES (#{marks})"
    ...
    def update!
      ...
      update_sql = "UPDATE #{@consulta} SET #{update_clause} WHERE id = ?"
